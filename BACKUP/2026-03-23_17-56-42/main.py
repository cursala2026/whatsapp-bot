...
def build_vendor_menu() -> str:
    vendedores_info = []
    for vid, vendedor in menu_config.get("vendedores", {}).items():
        cursos_asignados = sum(1 for c in menu_config.get("cursos", {}).values() if c.get("vendedor_id") == vid)
        nombre = f"{vendedor.get('nombre', '')} {vendedor.get('apellido', '')}"
        vendedores_info.append(f"{vid}. {nombre} ({cursos_asignados} cursos)")
    
    menu_str = "*GESTION DE VENDEDORES Y CURSOS*\n\n"
    menu_str += "📋 VENDEDORES ACTUALES:\n"
    for info in vendedores_info:
        menu_str += f"  {info}\n"
    menu_str += "\n1. Asignar cursos a vendedor\n"
    menu_str += "2. Ver cursos por vendedor\n"
    menu_str += "\n0. Volver al menu admin"
    return menu_str


def build_assign_course_menu() -> str:
    menu_str = "*ASIGNAR CURSO A VENDEDOR*\n\n"
    menu_str += "Vendedores disponibles:\n"
    for vid, vendedor in menu_config.get("vendedores", {}).items():
        nombre = f"{vendedor.get('nombre', '')} {vendedor.get('apellido', '')}"
        menu_str += f"{vid}. {nombre}\n"
    menu_str += "\nElegí el número del vendedor (o 0 para volver):"
    return menu_str


def build_courses_for_vendor_assignment(vendedor_id: str) -> str:
    vendedor = menu_config.get("vendedores", {}).get(vendedor_id, {})
    nombre = f"{vendedor.get('nombre', '')} {vendedor.get('apellido', '')}"
    
    menu_str = f"*ASIGNAR CURSOS A {nombre.upper()}*\n\n"
    menu_str += "Cursos disponibles:\n"
    
    for cid in sorted(menu_config.get("cursos", {}).keys(), key=int):
        curso = menu_config["cursos"][cid]
        asignado = curso.get("vendedor_id") == vendedor_id
        estado = "✅" if asignado else "⬜"
        menu_str += f"{cid}. {estado} {curso.get('nombre', '')}\n"
    
    menu_str += "\nEscribí el número del curso para asignarlo/desasignarlo"
    menu_str += "\n(o 0 para volver):"
    return menu_str


def build_vendor_courses_menu(vendedor_id: str) -> str:
    vendedor = menu_config.get("vendedores", {}).get(vendedor_id, {})
    nombre = f"{vendedor.get('nombre', '')} {vendedor.get('apellido', '')}"
    
    cursos_vendedor = [
        (cid, curso) for cid, curso in menu_config.get("cursos", {}).items()
        if curso.get("vendedor_id") == vendedor_id
    ]
    
    menu_str = f"*CURSOS ASIGNADOS A {nombre.upper()}*\n\n"
    
    if cursos_vendedor:
        for cid, curso in sorted(cursos_vendedor, key=lambda x: int(x[0])):
            menu_str += f"📌 {cid}. {curso.get('nombre', '')}\n"
    else:
        menu_str += "Sin cursos asignados\n"
...
def manejar_admin(from_number: str, text_body: str):
    ...
    if session["pending_action"] == "vendor_menu":
        ...
        if text == "1":
            enviar_respuesta(from_number, build_assign_course_menu())
            session["pending_action"] = "select_vendor_for_assignment"
            return
        
        if text == "2":
            enviar_respuesta(from_number, build_assign_course_menu())
            session["pending_action"] = "select_vendor_for_view"
            return
        
        enviar_respuesta(from_number, "❌ Opción inválida.\n\n" + build_vendor_menu())
        return

    if session["pending_action"] == "select_vendor_for_assignment":
        if text == "0":
            enviar_respuesta(from_number, build_vendor_menu())
            session["pending_action"] = "vendor_menu"
            return
        ...
        if text not in menu_config.get("vendedores", {}):
            enviar_respuesta(from_number, "❌ Vendedor no encontrado.\n\n" + build_assign_course_menu())
            return
        
        session["temp_vendor_id"] = text
        enviar_respuesta(from_number, build_courses_for_vendor_assignment(text))
        session["pending_action"] = "assign_course_to_vendor"
        return

    if session["pending_action"] == "assign_course_to_vendor":
        if text == "0":
            session["temp_vendor_id"] = None
            enviar_respuesta(from_number, build_vendor_menu())
            session["pending_action"] = "vendor_menu"
            return
        
        if text not in menu_config.get("cursos", {}):
            enviar_respuesta(from_number, "❌ Curso no encontrado.\n\n" + build_courses_for_vendor_assignment(session.get("temp_vendor_id", "1")))
            return
        
        vendor_id = session.get("temp_vendor_id")
        curso = menu_config["cursos"][text]
        nombre_curso = curso.get("nombre", f"Curso {text}")
        
        if curso.get("vendedor_id") == vendor_id:
            # Desasignar
            curso["vendedor_id"] = "1"  # Volver al vendedor por defecto
            accion = "desasignado de"
        else:
            # Asignar
            curso["vendedor_id"] = vendor_id
            accion = "asignado a"
        
        save_menu_config(menu_config)
        vendedor = menu_config.get("vendedores", {}).get(vendor_id, {})
        nombre_vendedor = f"{vendedor.get('nombre', '')} {vendedor.get('apellido', '')}"
        
        enviar_respuesta(
            from_number,
            f"✅ '{nombre_curso}' ha sido {accion} {nombre_vendedor}.\n\n"
            + build_courses_for_vendor_assignment(vendor_id)
        )
        return

    if session["pending_action"] == "select_vendor_for_view":
        if text == "0":
            enviar_respuesta(from_number, build_vendor_menu())
            session["pending_action"] = "vendor_menu"
            return
        ...
        enviar_respuesta(from_number, build_vendor_courses_menu(text))
        session["pending_action"] = "vendor_courses_view"
        session["temp_vendor_id"] = text
        return

    if session["pending_action"] == "vendor_courses_view":
        if text == "0":
            session["temp_vendor_id"] = None
            enviar_respuesta(from_number, build_vendor_menu())
            session["pending_action"] = "vendor_menu"
        ...
        enviar_respuesta(from_number, "❌ Opción inválida.\n\n" + build_vendor_courses_menu(session.get("temp_vendor_id", "1")))
        return
