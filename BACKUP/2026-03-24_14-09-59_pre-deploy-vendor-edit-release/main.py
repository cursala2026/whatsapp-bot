...
def build_runtime_revision_message() -> str:
    ...
    return (
        ...
        f"Servicio runtime: {runtime_service}\n"
        f"Revisión deploy: {runtime_revision}\n"
        f"Configuración deploy: {runtime_configuration}\n\n"
        "*Revisión de menú/config*\n"
        f"Versión menú: {menu_version}\n"
        f"Fecha último cambio menú: {menu_fecha}\n"
        f"Hora último cambio menú: {menu_hora}\n\n"
...
def build_vendor_edit_fields_menu(vendor_id: str) -> str:
    ...
    return (
        f"*EDITAR VENDEDOR*\n\n"
        f"Vendedor: {vendor_id}. {nombre}\n"
        f"Correo actual: {vendedor.get('correo', '')}\n"
        f"Telefono actual: {vendedor.get('telefono', '')}\n\n"
...
def build_vendor_add_confirmation(vendor_draft: dict) -> str:
    return (
        ...
        f"Nombre completo: {vendor_draft.get('full_name', '')}\n"
        f"Correo: {vendor_draft.get('correo', '')}\n"
        f"Telefono: {vendor_draft.get('telefono', '')}\n\n"
        "1. Guardar\n"
        "2. Editar\n"
        "0. Cancelar"
    )
