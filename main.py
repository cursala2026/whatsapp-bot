...
def load_menu_config() -> dict:
    default_config = {
        "greeting": "Hola 👋 Bienvenido/a a Cursala.\nGracias por comunicarte con nosotros.\n\n¿Cómo podemos ayudarte hoy?",
        "options": {
        ...
        "responses": {
            ...
            "2": "Excelente. En Cursala también ofrecemos capacitaciones para empresas.\n\nSi querés avanzar, contanos el nombre de tu empresa y la temática de interés.",
        ...
        "cursos": {
            "1": {"nombre": "Minería", "descripcion": "Formación en técnicas de extracción, seguridad minera.", "link_web": "https://www.cursala.com/cursos/mineria", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "2": {"nombre": "Soldadura", "descripcion": "Cursos de soldadura MIG, TIG y SMAW.", "link_web": "https://www.cursala.com/cursos/soldadura", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "3": {"nombre": "Piping", "descripcion": "Diseño e instalación de sistemas de tuberías.", "link_web": "https://www.cursala.com/cursos/piping", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "4": {"nombre": "Redes y telecomunicaciones", "descripcion": "Formación en redes y tecnología IT.", "link_web": "https://www.cursala.com/cursos/redes", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "5": {"nombre": "Instrumentación y control", "descripcion": "Cursos de automatización e instrumentación.", "link_web": "https://www.cursala.com/cursos/instrumentacion", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "6": {"nombre": "Herramientas para pymes", "descripcion": "Capacitación para pequeñas y medianas empresas.", "link_web": "https://www.cursala.com/cursos/pymes", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "7": {"nombre": "Ensayos No destructivos", "descripcion": "Técnicas avanzadas de inspección.", "link_web": "https://www.cursala.com/cursos/ensayos", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "8": {"nombre": "Diseño mecánico", "descripcion": "Formación en CAD y diseño mecanico.", "link_web": "https://www.cursala.com/cursos/diseno", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
            "9": {"nombre": "Logística para Pymes", "descripcion": "Gestión de cadena de suministro.", "link_web": "https://www.cursala.com/cursos/logistica", "link_descarga": "https://drive.google.com/ejemplo", "vendedor_id": "1"},
        },
        "vendedores": {
            "1": {"nombre": "Carlos", "apellido": "García", "telefono": "+5492615031839", "correo": "carlos@cursala.com.ar"}
        }
    }
    ...
    try:
        ...
        if missing_keys:
            ...
            # Usar defaults para las claves faltantes
            for key in missing_keys:
                config[key] = default_config[key]
            # Guardar la config corregida
...
def reorganize_course_ids():
    """Reorganiza los IDs de los cursos para mantener correlatividad (1, 2, 3, etc.)"""
    if not menu_config.get("cursos"):
        return
    
    # Obtener los cursos ordenados por ID actual
    cursos_ordenados = []
    for key in sorted(menu_config["cursos"].keys(), key=int):
        cursos_ordenados.append(menu_config["cursos"][key])
    
    # Reconstruir con IDs correlativas
    menu_config["cursos"] = {}
...
def manejar_usuario(from_number: str, text_body: str):
    ...
    # Si está viendo detalle de un curso
    if session["in_course_detail"]:
        ...
        if text == "0":
            ...
            msg = f"📞 Contacta a:\n{vendedor.get('nombre', 'N/A')} {vendedor.get('apellido', 'N/A')}\n📱 {vendedor.get('telefono', 'N/A')}\n📧 {vendedor.get('correo', 'N/A')}"
            enviar_respuesta(from_number, msg)
        else:
            enviar_respuesta(from_number, "Opción inválida.\n\n" + build_course_detail_menu(curso_id))
        return

    # Si está en el menú de cursos
    ...
    # Menú principal - comandos numéricos
    if text == "1":
        session["in_course_menu"] = True
        enviar_respuesta(from_number, build_courses_menu())
        return

    # Opciones 2, 3, 4 - respuestas predefinidas con opción de volver
    ...
    # Si está viendo una respuesta, puede volver
...
def manejar_admin(from_number: str, text_body: str):
    ...
    if session["pending_action"] == "confirm_course_data":
        if text == "1":
            ...
            enviar_respuesta(from_number, f"✅ Curso '{session['temp_course_data']['nombre']}' agregado exitosamente con ID {nuevo_id}.\n\n" + build_courses_edit_menu())
    ...
    if session["pending_action"] == "delete_course":
        if text in menu_config["cursos"]:
            ...
            enviar_respuesta(from_number, f"⚠️ ¿Estás seguro de eliminar '{curso['nombre']}'?\n\n1. ✅ Sí\n0. ❌ No\n\nEscribe tu opción:")
    ...
    if session["pending_action"] == "confirm_delete_course":
        if text == "1":
            ...
            enviar_respuesta(from_number, f"✅ Curso '{curso['nombre']}' eliminado exitosamente.\n\nℹ️ Los IDs se han reorganizado automáticamente.\n\n" + build_courses_edit_menu())
        elif text == "0":
    ...
    if session["pending_action"] == "edit_course_field":
        ...
        if text == "0":
            ...
            field_name = {"nombre": "nombre", "descripcion": "descripción", "link_web": "link web", "link_descarga": "link de descarga"}
    ...
    # Si está en el menú de editar cursos
    ...
    # Opción 1: Ver menú actual
    if text == "1":
        enviar_respuesta(from_number, "📋 " + build_main_menu())
        return

    # Opción 2: Modificar saludo
    if text == "2":
    ...
    # Opción 3: Editar opción del menú
    if text == "3":
    ...
    # Opción 6: Editar cursos
    if text == "6":
        session["in_courses_edit_menu"] = True
        enviar_respuesta(from_number, build_courses_edit_menu())
        return

    # Opción 7: Gestionar vendedores
    if text == "7":
    ...
    # Opción 9: Desactivar admin
    if text == "9":
        session["active"] = False
        enviar_respuesta(from_number, "✅ Admin desactivado.")
        return

    # Procesar acciones pendientes de admin
    ...
    if session["pending_action"] == "edit_response_text":
        ...
        session["change_history"].append(f"Respuesta {response_id}: '{menu_config['responses'][response_id][:40]}...' → '{text_body[:40]}...'")
    ...
    if session["pending_action"] == "edit_vendor_field":
        ...
        if text == "0":
            ...
            session["temp_field"] = fields[text]
            field_names = {"nombre": "Nombre", "apellido": "Apellido", "telefono": "Teléfono", "correo": "Correo"}
    ...
    if session["pending_action"] == "delete_vendor":
        if text in menu_config["vendedores"]:
            vendor = menu_config["vendedores"][text]
            session["temp_option"] = text
            enviar_respuesta(from_number, f"⚠️ ¿Estás seguro de eliminar '{vendor['nombre']} {vendor['apellido']}'?\n\n1. ✅ Sí\n0. ❌ No")
